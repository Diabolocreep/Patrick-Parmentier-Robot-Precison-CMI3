...
void Liste::enfiler(String V)
{
  if (tete == nullptr)
  {
	...
	temp->suiv = new Maillon(V);
}
String Liste::defiler()
{
	if (tete == nullptr) 
  {
    return "C";
  ...
  {
    String instruction = tete->suiv->getInstr();
    delete tete;
    tete = nullptr;
    return instruction;
  }
  ...
  String instruction = temp->suiv->getInstr();
  delete temp;
  temp = nullptr;
  return instruction;
}
void Liste::clearFile()
{
  if (tete != nullptr) {
    delete tete;
    tete = nullptr;
